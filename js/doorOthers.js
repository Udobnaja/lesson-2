...
class Door2 extends DoorBase {
    constructor(...args) {
        ...
        for (let gear of this.gears) {
            gear.addEventListener('pointerdown', this._onGearPointerDown.bind(this));
            gear.addEventListener('pointerup', this._onGearPointerUp.bind(this));
            // gear.addEventListener('pointerleave', this._onGearPointerUp);
            gear.addEventListener('pointercancel', this._onGearPointerUp.bind(this));
    ...
    _onGearPointerMove(e) {
        ...
        let posY = target.parentElement.offsetHeight -
                  (e.clientY - target.parentElement.offsetTop - (target.offsetHeight / 2));
    ...
    _onLeverPointerMove(e) {
        ...
        if ((target.hasPointerCapture && target.hasPointerCapture(e.pointerId)) ||
              target.classList.contains('lever__button_pressed')) {
...
class Box extends DoorBase {
    ...
    _onKeyPointerMove(e) {
        ...
        if (target.getBoundingClientRect().top <= target.parentElement.parentElement.offsetTop +
          this.popup.querySelector('.keypanel__keyhole').offsetTop) {
    ...
    _onPanelPointerMove(e) {
        ...
        this.panels.forEach((p) => {
            if (p.dataset.pos === 'left') {
                left = p.parentElement.offsetWidth / 2 + p.parentElement.getBoundingClientRect().left -
                  p.getBoundingClientRect().left - p.offsetWidth;
            } else {
                right = p.getBoundingClientRect().left - p.parentElement.getBoundingClientRect().left -
                  p.parentElement.offsetWidth / 2;
            }
